...
class ReservationRedisProjectorAdapterTest {
	...
	@Test
	void ReservationRedisProjectorAdapter_existsReservationRedisEntityByClientId_ReturnBoolFalse() {
		
		Mockito.when(reservationRedisRepository.findByClientId(Mockito.any(UUID.class))).thenReturn(new ArrayList<>());

		boolean resultValue = reservationRedisProjectorAdapter.existsReservationRedisEntityByClientId(clientId);
		
		org.junit.jupiter.api.Assertions.assertAll(
				()-> Assertions.assertThat(resultValue).isFalse()
				);
		Mockito.verify(reservationRedisRepository, times(1)).findByClientId(Mockito.any(UUID.class));
		Mockito.verify(loggerUtil, times(1)).logInfoAction(Mockito.anyString(), Mockito.anyString(), Mockito.anyString());	
	}

	@Test
	void ReservationRedisProjectorAdapter_existsReservationRedisEntityByClientId_ReturnBoolTrue() {
		
		Mockito.when(reservationRedisRepository.findByClientId(Mockito.any(UUID.class))).thenReturn(List.of(new ReservationRedisEntity()));
