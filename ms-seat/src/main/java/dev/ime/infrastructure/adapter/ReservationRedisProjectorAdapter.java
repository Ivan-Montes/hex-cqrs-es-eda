...
public class ReservationRedisProjectorAdapter implements ReservationRedisProjectorPort {
	...
	@Override
	public boolean existsReservationRedisEntityBySeatId(UUID seatId) {
		
        ReservationRedisEntity probe = new ReservationRedisEntity();
        probe.setSeatIdsSet(Set.of(seatId));

        ExampleMatcher matcher = ExampleMatcher.matchingAny()
                .withMatcher("seatIdsSet", ExampleMatcher.GenericPropertyMatchers.contains());

        Example<ReservationRedisEntity> example = Example.of(probe, matcher);
        var result = reservationRedisRepository.findAll(example);
