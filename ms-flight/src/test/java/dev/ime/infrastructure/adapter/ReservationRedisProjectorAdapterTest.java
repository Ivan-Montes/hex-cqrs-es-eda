...
class ReservationRedisProjectorAdapterTest {
	...
	@Test
	void ReservationRedisProjectorAdapter_existsReservationRedisEntityByFlighId_ReturnBoolFalse() {
		
		Mockito.when(reservationRedisRepository.findByFlightId(Mockito.any(UUID.class))).thenReturn(new ArrayList<>());

		boolean resultValue = reservationRedisProjectorAdapter.existsReservationRedisEntityByFlighId(flightId);
		
		org.junit.jupiter.api.Assertions.assertAll(
				()-> Assertions.assertThat(resultValue).isFalse()
				);
		Mockito.verify(reservationRedisRepository, times(1)).findByFlightId(Mockito.any(UUID.class));
		Mockito.verify(loggerUtil, times(1)).logInfoAction(Mockito.anyString(), Mockito.anyString(), Mockito.anyString());	
	}

	@Test
	void ReservationRedisProjectorAdapter_existsReservationRedisEntityByFlighId_ReturnBoolTrue() {
		
		Mockito.when(reservationRedisRepository.findByFlightId(Mockito.any(UUID.class))).thenReturn(List.of(new ReservationRedisEntity()));
